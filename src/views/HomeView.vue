<template>
  <main class="home-view">
    <section class="hero" id="home">
      <div class="hero-inner container">
        <div class="hero-text full">
          <h2>안녕하세요<br />끈기 있게 배우며 성장하는<br />백엔드 개발자 안태민입니다.</h2>
    ...
    <section class="resume container" id="aboutme">
      <h2>About Me</h2>
      <div class="about-me">
        <div class="about-me-inner">
          <p class="about-bio">저는 신입 백엔드 개발자로서
          ...
          <ul class="contact-icons">
            ...
            <li>
              <a class="icon-link linkedin" href="https://www.linkedin.com/in/%ED%83%9C%EB%AF%BC-%EC%95%88-244782384/" target="_blank" rel="noopener" aria-label="LinkedIn" title="LinkedIn">
                <svg viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M4.98 3.5C3.88 3.5 3 4.38 3 5.48c0 1.1.88 1.98 1.98 1.98h.02C6.08 7.46 6.96 6.58 6.96 5.48 6.96 4.38 6.08 3.5 4.98 3.5zM3.5 8.98h3v11h-3v-11zM10.5 8.98h2.86v1.5h.04c.4-.76 1.38-1.56 2.84-1.56 3.04 0 3.6 2 3.6 4.59v6.47h-3v-5.74c0-1.37-.03-3.13-1.9-3.13-1.9 0-2.19 1.48-2.19 3.01v5.86h-3v-11z"/></svg>
                <span class="icon-label">LinkedIn</span>
              </a>
            </li>
            <li>
              <a class="icon-link velog" href="https://velog.io/@taemin01/posts" target="_blank" rel="noopener" aria-label="Velog" title="Velog">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M7 7h10M7 11h10M7 15h6" stroke-linecap="round" stroke-linejoin="round"/><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg>
                  <span class="icon-label">Velog</span>
                </a>
            </li>
            <li>
              <a class="icon-link instagram" href="https://www.instagram.com/t._.m2_/" target="_blank" rel="noopener" aria-label="Instagram" title="Instagram">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" stroke-linecap="round" stroke-linejoin="round"/><path d="M17.5 6.5h.01" stroke-linecap="round" stroke-linejoin="round"/></svg>
    ...
    <section class="about container" id="skills">
      <h2>Skills</h2>
    ...
    <section class="projects container" id="projects">
      ...
      <div class="cards">
        ...
        >
          <h3 class="card-title">
            ...
            <router-link class="card-link" :to="`/projects/${project.id}`">자세히 보기</router-link>
          </h3>
          <p class="card-summary">{{ project.cardSummary }}</p>
          <p v-if="project.cardHighlight" class="card-highlight">{{ project.cardHighlight }}</p>

          <div class="tags" v-if="getTags(project).length">
            <span class="tag" v-for="(t, i) in getTags(project)" :key="i">{{ t }}</span>
          </div>
      ...
      <div class="project-toggle" v-if="projects.length &gt; visibleCount" @click="toggleExpand" role="button" tabindex="0" @keyup.enter="toggleExpand">
        <span class="toggle-label">{{ expanded ? '프로젝트 접기' : '프로젝트 더보기' }}</span>
        <svg class="toggle-arrow" :class="{ open: expanded }" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M6 9l6 6 6-6" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </div>
    </section>
  </main>
</template>
...
export default {
  ...
  computed: {
    displayProjects() {
      if (this.expanded) return this.projects
  ...
  methods: {
    ...
    getTags(project) {
      ...
      // collect arrays from stacks object and flatten
      const tags = []
...
.home-view h2 { text-align: left; margin: 0 0 16px; }
...
.skill-card h3, .skill-card h4 { margin:0 0 8px }
.skill-list { margin:0; color:var(--muted); font-size:15px; font-weight:400 }
.skill-note { margin:6px 0 0; font-size:15px; color:var(--muted) }
.skill-card.full-width { grid-column: 1 / -1 }

@media (max-width: 720px) {
  .skills-grid { grid-template-columns: 1fr }
  .skill-list, .skill-note { font-size:14px }
}
.projects { margin-top:0; background:var(--page-bg); padding:32px 0 }
.cards { display:grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap:16px; margin-top:8px }
.card { background:var(--ui-bg); border:1px solid var(--card-border); padding:16px; border-radius:8px; text-align:left; box-shadow: 0 1px 0 rgba(15,23,42,0.02); box-sizing:border-box; position:relative }
.card h3 { color:var(--text); margin:0 0 8px }
.card .link { color:var(--primary); text-decoration: none }
.card-summary { margin:0 0 8px; color:var(--muted); font-size:15px; font-weight:400 }
...
.card-title-text { font-weight:600; display:block }
.card-link { position:absolute; top:16px; right:16px; font-size:13px; color:var(--primary); text-decoration:none }
...
/* About Me styles */
.about-me { padding: 12px 0 0; }
.about-me-inner { display:flex; flex-direction:column; gap:12px; align-items:flex-start; }
.about-bio { margin:0 0 12px; color:var(--text); max-width:720px; line-height:1.6; color: black;}
.contact-icons { list-style:none; margin:0; padding:0; display:flex; flex-direction:column; gap:12px; align-items:flex-start }
.contact-icons li { display:block }
.icon-link { display:inline-flex; align-items:center; gap:12px; color:var(--primary); text-decoration:none; font-size:16px }
.icon-link svg { width:28px; height:28px; flex-shrink:0 }
.icon-label { display:inline-block }
.icon-link:hover, .icon-link:focus { color:var(--primary) }
/* Velog icon larger to avoid clipping */
.velog-icon { width:36px; height:36px; display:block; }
@media (max-width:720px) {
  .contact-icons { gap:10px }
  .icon-link svg { width:24px; height:24px }
  .icon-link { font-size:15px }
  .velog-icon { width:30px; height:30px }
  .card-summary, .card-highlight { font-size:14px }
  .cards { grid-template-columns: repeat(2, minmax(0, 1fr)) }
  .card-title { padding-right:0 }
  .card-link { position:static; display:inline-block; margin-left:8px }
}
.quick-tags { color:var(--primary); font-weight:600; margin-top:8px }
/* ensure sections don't get hidden under fixed header when navigating */
section { scroll-margin-top: var(--hf-height); }

@media (max-width: 720px) {
  .about-me-inner { flex-direction: column }
}
</style>
